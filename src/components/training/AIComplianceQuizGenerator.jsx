...
export default function AIComplianceQuizGenerator({ nurseEmail, _recommendations = [], initialTopicId = null }) {
  ...
  // Which topic's quiz is being generated. `ai.loading` is a single flag for the
  // whole hook, so keying the card spinner off it put "Generating..." on EVERY
  // topic card at once; only the card the user picked should show progress.
  const [generatingTopicId, setGeneratingTopicId] = useState(null);
  // Latch the topic we've already auto-generated so clearing `quiz` (via
  // "Try Different Topic") can't re-fire the effect and regenerate the same
  // topic — the topic picker must be reachable.
  const autoGeneratedTopicRef = React.useRef(null);
  // Read synchronously inside generateQuiz: React state has not flushed yet when
  // a second click lands in the same tick, so the state flag alone can't
  // de-duplicate a double tap.
  const generatingTopicIdRef = React.useRef(null);

  // Auto-generate quiz if topic provided (once per initialTopicId value)
  React.useEffect(() => {
    if (initialTopicId && autoGeneratedTopicRef.current !== initialTopicId) {
      const topic = quizTopics.find(t => t.id === initialTopicId);
      if (topic) {
        autoGeneratedTopicRef.current = initialTopicId;
        generateQuiz(topic);
      }
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps -- run-once auto-generate per initialTopicId; a ref latch prevents re-fire when quiz is cleared
  }, [initialTopicId]);
  ...
  const generateQuiz = async (topic) => {
    if (generatingTopicIdRef.current) return; // one generation at a time
    generatingTopicIdRef.current = topic.id;
  ...
  "questions": [
    {
      ...
      generatingTopicIdRef.current = null;
              ...
              >
                <CardContent className="p-4">
                  ...
                  <Button
                    size="sm"
                    className="w-full"
                    // Its own handler, stopping propagation so the card's
                    // onClick can't also fire and start a second generation.
                    onClick={(e) => { e.stopPropagation(); generateQuiz(topic); }}
