...
export default function CompletionTracker({ moduleId, moduleTitle }) {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'assigned': return <AlertCircle className="w-4 h-4 text-slate-400" />;
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Completion Tracking
          {moduleTitle && <span className="text-sm font-normal text-slate-500">- {moduleTitle}</span>}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div>
          <h4 className="text-sm font-semibold text-slate-700 mb-3">Individual Progress</h4>
          <ScrollArea className="h-[400px]">
            <div className="space-y-2">
              {completions.map((completion) => {
                const user = allUsers.find(u => u.email === completion.nurse_email);
                return (
                  <div key={completion.id} className="flex items-center justify-between p-3 bg-white rounded-lg border border-slate-200">
