...
export default function ClinicalSkillsChecklist() {
  ...
  const filteredMembers = useMemo(() => teamMembers.filter((user) => {
    const query = search.toLowerCase();
    return !query || user.full_name?.toLowerCase().includes(query) || user.email?.toLowerCase().includes(query);
  }), [teamMembers, search]);

  const selectedEmployee = filteredMembers.find((user) => user.email === selectedEmail) || filteredMembers[0];

  if (currentUser && !canSupervise(currentUser)) {
    return <div className="max-w-3xl mx-auto p-6 text-slate-600">This clinical skills checklist module is available to supervisors, managers, and admins only.</div>;
  }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[320px_minmax(0,1fr)] gap-6">
        ...
        {selectedEmployee ? (
          <EmployeeCompetencyProfile employee={selectedEmployee} currentUser={currentUser} />
        ) : (
          <Card><CardContent className="p-10 text-center text-slate-500">No employees available for observation.</CardContent></Card>
        )}
      </div>
    </div>
  );
