...
/**
 * reviewTimesheet — approve or reject a submitted timesheet.
 *
 * Authorization is enforced server-side: only an admin or the timesheet's
 * designated manager may review, and a reviewer can never act on their own
 * timesheet (no self-approval). The status write uses the service role because
 * the Timesheet entity limits direct writes to admins. An approved timesheet is
 * what feeds the final payroll export.
 */
...
Deno.serve(async (req) => {
  try {
    ...
    if (!timesheet_id) {
      return Response.json({ error: 'timesheet_id is required.' }, { status: 400 });
    }
    if (!['approved', 'rejected'].includes(decision)) {
      return Response.json({ error: 'decision must be "approved" or "rejected".' }, { status: 400 });
    }

    const timesheet = await base44.asServiceRole.entities.Timesheet.get(timesheet_id).catch(() => null);
    if (!timesheet) {
    ...
    // Agency-scoped admins may only review timesheets for staff in their agency.
    ...
    const updated = await base44.asServiceRole.entities.Timesheet.update(timesheet_id, {
      status: decision,
    ...
    try {
      ...
      const period = `${timesheet.pay_period_start} → ${timesheet.pay_period_end}`;
      const approved = decision === 'approved';
      await base44.asServiceRole.entities.Notification.create({
        user_email: timesheet.employee_email,
        title: approved ? 'Timesheet approved' : 'Timesheet needs changes',
        message: `Your timesheet for ${period} was ${decision}${trimmedNote ? `: ${trimmedNote}` : '.'}`,
