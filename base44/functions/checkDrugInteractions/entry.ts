...
// Inline mirror of src/components/medication/drugInteractions.js. Base44 edge
// functions deploy as standalone files, so this deterministic safety net is
// guarded by base44/functions/drugInteractionsInlineParity.test.js.
const GROUPS = {
  ...
  const matchesFragment = (fragment) => {
    if (fragment.includes(' ')) {
      ...
      for (let i = 0; i + words.length <= tokens.length; i++) {
        if (words.every((word, index) => tokens[i + index] === word)) return true;
      }
      return false;
    }
    return tokenSet.has(fragment);
  };
  const out = [];
...
function findDeterministicInteractions(medications) {
  const meds = (medications || [])
    .map((medication) => ({ name: medication?.medication_name || medication?.name || '' }))
    .filter((medication) => medication.name);
  const memberGroups = meds.map((medication) => new Set(groupsFor(medication.name)));
  const results = [];
  const seen = new Set();
...
function mergeInteractions(aiInteractions, deterministic) {
  ...
  const pairKey = (interaction) => [norm(interaction.drug_a), norm(interaction.drug_b)].sort().join('|');
  const detKeys = new Set(deterministic.map(pairKey));
  const aiTagged = (aiInteractions || [])
    .filter((interaction) => !detKeys.has(pairKey(interaction)))
    .map((interaction) => ({ ...interaction, source: interaction.source || 'ai_suggested', verified: false }));
...
Deno.serve(async (req) => {
  try {
    ...
    const medications = Array.isArray(body.medications)
      ? body.medications
      : Array.isArray(body.current_medications)
        ? body.current_medications
        : [];

    if (!medications.length) {
      return Response.json({ interactions: [], deterministic_interactions: [], count: 0 });
    }

    const deterministic = findDeterministicInteractions(medications);
    let aiInteractions = [];

    if (body.skip_ai !== true && body.use_ai !== false) {
      try {
        const raw = await base44.integrations.Core.InvokeLLM({
          prompt: `Check these medications for clinically significant drug-drug interactions. Return ONLY JSON with {"interactions":[{"drug_a":"","drug_b":"","severity":"minor|moderate|major|critical","interaction_type":"","description":"","clinical_significance":"","recommendation":"","monitoring_required":true,"requires_intervention":false}]}.\n\nMEDICATIONS:\n${JSON.stringify(medications, null, 2)}`,
        });
        const parsed = parseLLMJson(raw);
        aiInteractions = Array.isArray(parsed?.interactions) ? parsed.interactions : [];
      } catch {
        aiInteractions = [];
      }
    }

    const interactions = mergeInteractions(aiInteractions, deterministic);
    return Response.json({
      interactions,
      deterministic_interactions: deterministic,
      count: interactions.length,
    });
  } catch (error) {
    return Response.json({ error: error.message || 'Failed to check drug interactions' }, { status: 500 });
  }
});
